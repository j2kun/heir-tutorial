import os
import time
from typing import List
import numpy as np
import torch
from torch.utils.data import DataLoader, Dataset
import fashion_mnist_pybind as fashion_mnist


DATA_PATH = "data"


class CustomMNISTTestDataset(Dataset):
    """This custom dataset loads the raw MNIST test data and labels

    from the files specified by `data_root`.
    It applies the Normalize transform manually during loading.
    """

    def __init__(
        self,
        data_root: str,
        normalize_mean: float = 0.1307,
        normalize_std: float = 0.3081,
    ):
        self.data_root = data_root
        self.normalize_mean = normalize_mean
        self.normalize_std = normalize_std

        labels_path = os.path.join(self.data_root, "t10k-labels-idx1-ubyte")
        with open(labels_path, "rb") as f:
            # Skip header
            f.read(8)
            labels = np.frombuffer(f.read(), dtype=np.uint8)
            self.targets = torch.tensor(labels, dtype=torch.long)

        images_path = os.path.join(self.data_root, "t10k-images-idx3-ubyte")
        with open(images_path, "rb") as f:
            # Skip header
            f.read(16)
            images = np.frombuffer(f.read(), dtype=np.uint8)
            images = images.reshape(-1, 1, 28, 28)
            self.data = torch.tensor(images, dtype=torch.float32) / 255.0

    def __len__(self) -> int:
        return len(self.data)

    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor]:
        image = self.data[index]
        label = self.targets[index]

        # Apply normalization manually (C++ .map(transforms::Normalize))
        # (X - mean) / std_dev
        image = (image - self.normalize_mean) / self.normalize_std

        return image, label





def main():

    test_dataset = CustomMNISTTestDataset(data_root=DATA_PATH)
    test_loader = DataLoader(
        test_dataset,
        batch_size=1,
        shuffle=False,  # SequentialSampler equivalent
    )

    print("Timing generated functions...")

    start_time = time.time()
    crypto_context = fashion_mnist.fashion_mnist__generate_crypto_context()
    print(f"fashion_mnist__generate_crypto_context took: {(time.time() - start_time)*1000:.2f} ms")

    key_pair = crypto_context.KeyGen()
    public_key = key_pair.publicKey
    secret_key = key_pair.secretKey

    start_time = time.time()
    crypto_context = fashion_mnist.fashion_mnist__configure_crypto_context(crypto_context, secret_key)
    print(f"fashion_mnist__configure_crypto_context took: {(time.time() - start_time)*1000:.2f} ms")

    # Evaluation Loop
    total = 4
    samples_processed = 0

    results = []

    for batch_data, batch_target in test_loader:
        if samples_processed >= total:
            break

        print(f"\nProcessing sample {samples_processed + 1}/{total}")

        input_tensor = batch_data.contiguous()  # (1, 1, 28, 28)
        input_vector = input_tensor.flatten().tolist()

        start_time = time.time()
        input_encrypted = fashion_mnist.fashion_mnist__encrypt__arg0(
            crypto_context, input_vector, public_key
        )
        print(f"  fashion_mnist__encrypt__arg0 took: {(time.time() - start_time)*1000:.2f} ms")

        start_time = time.time()
        output_encrypted = fashion_mnist.fashion_mnist(crypto_context, input_encrypted)
        print(f"  fashion_mnist (inference) took: {(time.time() - start_time)*1000:.2f} ms")

        start_time = time.time()
        output = fashion_mnist.fashion_mnist__decrypt__result0(
            crypto_context, output_encrypted, secret_key
        )
        print(f"  fashion_mnist__decrypt__result0 took: {(time.time() - start_time)*1000:.2f} ms")

        label = batch_target.item()
        max_id = max(range(len(output)), key=lambda index: output[index])

        results.append({
            "logits": output,
            "prediction": max_id,
            "label": label
        })

        samples_processed += 1

    print("\n=== Final Results ===")
    for i, res in enumerate(results):
        print(f"\nSample {i}:")
        print(f"  Logits: {res['logits']}")
        print(f"  Prediction: {res['prediction']}")
        print(f"  Label: {res['label']}")
        print(f"  Match: {res['prediction'] == res['label']}")


if __name__ == "__main__":
    main()
